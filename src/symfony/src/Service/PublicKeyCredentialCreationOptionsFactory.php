...
final class PublicKeyCredentialCreationOptionsFactory implements CanDispatchEvents
{
    ...
    /**
     * The deprecated `webauthn.creation_profiles.*.rp.name` node is passed as is. When it is omitted, the entity keeps
     * an empty name and PublicKeyCredentialRpEntityDenormalizer defaults it to the Relying Party ID, so a profile that
     * dropped the deprecated node no longer triggers the deprecation of the entity itself.
     *
