...
class WebauthnListener
{
    ...
    public function __invoke(RequestEvent $event): void
    {
        ...
        if (mb_strpos($content, 'json') === false && mb_strpos($request->getRequestFormat(), 'json') === false) {
        ...
        switch (true) {
            case $this->options['authentication']['enabled'] === true && $this->httpUtils->checkRequestPath(
                $request,
                $this->options['authentication']['routes']['result_path']
            ):
            ...
            case $this->options['authentication']['enabled'] === true && $this->httpUtils->checkRequestPath(
                $request,
                $this->options['authentication']['routes']['options_path']
            ):
                $this->logger->debug('The path corresponds to the request options path');
                $this->requestListener->processWithRequestOptions($event);

                return;
            case $this->options['registration']['enabled'] === true && $this->httpUtils->checkRequestPath(
                $request,
                $this->options['registration']['routes']['result_path']
            ):
            ...
            case $this->options['registration']['enabled'] === true && $this->httpUtils->checkRequestPath(
                $request,
                $this->options['registration']['routes']['options_path']
            ):
                ...
                return;
            default:
                $this->logger->debug('The path does not corresponds to any configured path. Ignored', [
