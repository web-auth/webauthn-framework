...
const moveTypescriptDeclarationsPlugin = (packagePath) => ({
    ...
    writeBundle: async () => {
        const files = glob.sync(path.join(packagePath, 'dist', '*', 'Resources', 'assets', 'src', '**/*.d.ts'));
        files.forEach((file) => {
            // a bit odd, but remove first 9 directories, which will leave
            // only the relative path to the file
            const relativePath = file.split('/').slice(9).join('/');
...
const files = glob.sync('src/*/Resources/assets/src/*controller.ts');
